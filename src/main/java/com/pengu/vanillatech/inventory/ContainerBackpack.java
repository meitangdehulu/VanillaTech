...
public class ContainerBackpack extends Container
{
	...
	public ContainerBackpack(EntityPlayer player, UUID backpack)
	{
		this.backpack = backpack;
		
		if(!player.world.isRemote)
			HammerCore.audioProxy.playSoundAt(player.world, "ui.toast.in", player.getPosition(), 1, 1, SoundCategory.PLAYERS);
		
		if(!player.getHeldItemMainhand().isEmpty() && player.getHeldItemMainhand().getItem() instanceof ItemBackpack && player.getHeldItemMainhand().hasTagCompound() && backpack.equals(player.getHeldItemMainhand().getTagCompound().getUniqueId("UUID")))
		{
			packStack = player.getHeldItemMainhand();
			hand = EnumHand.MAIN_HAND;
		} else if(!player.getHeldItemOffhand().isEmpty() && player.getHeldItemOffhand().getItem() instanceof ItemBackpack && player.getHeldItemOffhand().hasTagCompound() && backpack.equals(player.getHeldItemOffhand().getTagCompound().getUniqueId("UUID")))
		{
	...
	public void onContainerClosed(EntityPlayer playerIn)
	{
		...
		{
			HammerCore.audioProxy.playSoundAt(playerIn.world, "ui.toast.out", playerIn.getPosition(), 1, 1, SoundCategory.PLAYERS);
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
		...
		if(slot != null && slot.getHasStack())
		{
			...
			if(index < 54)
			{
				if(!this.mergeItemStack(itemstack1, 54, inventorySlots.size(), true))
